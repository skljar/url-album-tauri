...
| Диалоги | rfd 0.15 (`AsyncFileDialog`, везде с `set_parent` — см. «Активные проблемы» про DPI) |
...
- [ ] **DPI-баг от `set_parent` — НЕПРОВЕРЕННОЕ подозрение неизвестного происхождения.** Проверено по репозиторию (сессия 21): коммита, где бы `set_parent` снимали или чинили DPI, нет; в сообщениях коммитов слово «DPI» не встречается **ни разу**; `set_parent` менялся всего дважды — в первичном импорте `fb6c142` (2 июня) и в `7ff78d1` (3 июня), и оба раза вызовы только **добавлялись**. Утверждение, будто его «убрали из `open_db`», не подтверждается: `open_db` этого вызова никогда не имел. Сейчас `.set_parent(&window)` стоит в **15 местах**, включая `backup_db` (1270) и `backup_db_with_data` (1307), то есть все файловые диалоги ведут себя одинаково. Прежде чем что-то менять — воспроизвести: масштаб 125–150 %, открыть диалог, посмотреть на окно программы после закрытия. Без симптома правка беспредметна

### Архитектурные ограничения
- `rfd::AsyncFileDialog` — модальные диалоги, `set_parent` во всех 15 вызовах (прежняя запись про «убрано из `open_db`» была ошибочной, см. «Активные проблемы»)
...
- [ ] ~~Убрать `.set_parent(&window)` из `backup_db` для единообразия с `open_db`~~ — **пункт снят**: единообразие уже есть, вызов стоит везде, а «`open_db` без `set_parent`» оказалось ошибочной записью. Точечное снятие в одном месте как раз и создало бы разнобой. Если DPI-баг подтвердится — менять все 15 мест одной правкой
