...
- **Отказ логируется ОДИН раз, в Rust** (`open_url`, `open_url_with`). В JS не дублировать: `openWithBrowser` получает уже записанную причину и только показывает окно, иначе на одно событие приходятся две строки в журнале.
