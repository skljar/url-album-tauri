...
- **`open_file` НЕ возвращает ошибку на несуществующем пути.** Она делает `spawn` на `cmd /c start`, и сам запуск удаётся всегда — ошибка возникает уже внутри `cmd`, куда нам не видно. Поэтому отсутствие файла сообщает `get_log_path`, возвращая `None`, а UI показывает понятный текст. Полагаться на `Err` от `open_file` в подобных случаях нельзя.
