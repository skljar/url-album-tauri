...
### Прокси
- **Область действия — только `fetch_favicon` и `check_url`.** Скриншоты создаёт внешний headless-браузер: настройку он не видит, для него нужен флаг `--proxy-server`, а **авторизацию Chromium в этом флаге игнорирует** (потребуется локальный релей). Открытие ссылок в браузере — тоже мимо прокси.
- `build_proxy(host, port, user, pass)` — общая точка сборки для настроек и для проверки; срезает схему `http://`/`https://` и хвостовой слэш. Формат URL и правило basic-auth обязаны совпадать в обоих путях, иначе «Проверить» покажет успех для конфигурации, которой нет в бою
- `proxy_from_settings()` — читает `settings.json` **при каждом вызове**, то есть на каждый HTTP-запрос. Плата за то, что прокси подхватывается сразу после «Сохранить», без перезапуска
...
- [ ] Прокси для скриншотов: внешний headless-браузер настройку не видит. Без авторизации — флаг `--proxy-server=host:port`; с логином и паролем — только локальный релей, Chromium учётные данные в этом флаге игнорирует
