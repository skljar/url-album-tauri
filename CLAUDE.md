...
- Что логируется: старт (версия, путь к exe, конфигурация прокси — адрес и порт, без логина, версия WebView2), открытие базы, HTTP-сервер расширения, релей (соединение, отказ подключения, не-2xx на CONNECT), `do_screenshot` (команда и результат с длительностью), стратегия favicon, ошибки `check_url` (успехи — нет, иначе проверка папки раздувает файл), отказы WAL-checkpoint и записи `last_db.txt`/`recent_dbs.txt`
