...
- `proxy_server_arg() -> Option<String>` — `host:port` для флага `--proxy-server`; **возвращает `None`, если задан логин** (см. Критические ловушки)
...
- [ ] Скриншоты через прокси с логином и паролем — нужен локальный релей без авторизации, на который указывать `--proxy-server`. Chromium учётные данные в этом флаге игнорирует, поэтому иначе никак
...
- **`proxy_server_arg()` возвращает `None` при непустом логине — флаг `--proxy-server` НЕ передаётся, скриншот идёт напрямую.** Chromium игнорирует учётные данные в этом флаге: на 407 он показал бы диалог логина, которого в headless нет, и завершился бы молча, без PNG. Пойти напрямую хуже по приватности, но лучше, чем гарантированно остаться без скриншота. Прокси с авторизацией потребует локального релея.

- **Ошибки прокси в `do_screenshot` не диагностируются вовсе.** stdout/stderr headless-браузера не читаются, отдельного канала ошибок нет, решение принимается по факту наличия PNG — отсутствие файла неотличимо от «сайт не открылся». Единственная подсказка человеку — приписка «(включён прокси — проверьте его доступность)» в тексте ошибки, и вешается она по флагу `proxy_used`, то есть **только когда флаг реально передан**. При прокси с логином приписки нет намеренно: трафик шёл напрямую, и упоминание прокси было бы ложным следом. (Сессия 20, `718b9ca`.)
